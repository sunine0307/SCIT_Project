...
public class FileService {
   ...
   public static String saveFile(MultipartFile mfile, String uploadPath) {
      ...
      SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd"); //simpleDateFormat : toChar같은것
      String savedFilename = sdf.format(new Date());   //savedFilename에는 20200810
      
      //원본 파일의 확장자 : 날짜.jpg부분을 만들기 위해 확장자를 담아줄 변수 생성
      String ext;
      int lastIndex = originalFilename.lastIndexOf('.'); // text.jpg일 때 뒤에서부터 찾아서 4가 들어감
      //확장자가 없는 경우
      if (lastIndex == -1) {
         ext = "";
      }
      //확장자가 있는 경우
         ...
         // "." + jpg -> .jpg를 ext에 넣어줌
         ext = "." + originalFilename.substring(lastIndex + 1); // .뒤의 j부터 해당
      }
      ...
      //파일 저장
      try {
         // 파일객체 trasferTo 메소드를 실행할때 저장할 파일객체를 매개변수로 전달
         // 물리적인 저장(usb에서 파일 복사해서 c:붙여넣기) : 원본파일명이랑 다른 파일명이 저장됨
         mfile.transferTo(serverFile);
      } catch (Exception e) {
         savedFilename = null;
         e.printStackTrace();
      }
      // 논리적인 저장(DB에 xxx파일을 저장했다)
